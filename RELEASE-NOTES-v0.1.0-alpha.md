...
- Project save / load (versioned JSON). **12 bundled examples** in the Load → Examples menu (Two oscillators mixed, ADSR-shaped pulse, Kick drum, Snare drum, Bass lead, Lo-fi pad, Stair-stepped arpeggio, Echo, Lo-fi crunch, Color bars on a VGA monitor, White vertical stripe on a VGA monitor, CPU accumulator), Help → About, and a default starter graph on first launch so the canvas is never empty.
