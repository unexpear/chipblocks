...
Repo + installer: https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.7
...
https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.7
...
ChipBlocks is a free open-source desktop app that lets non-engineers design custom chips by dragging visual blocks. The first alpha covers audio / synth / retro-game chips — 40 blocks in a node-graph editor, simulated audio output, plus four real-silicon outputs: a flashable iCE40 bitstream for the $30 Lattice iCEstick, a bitstream for the TinyFPGA BX, a bitstream for the 1BitSquared iCEBreaker (~$70 with PMOD headers — and a working VGA path on PMOD1B: drop in a $8 VGA-PMOD attachment, build the bundled `color-bars.json` graph, see 8 SMPTE color bars on a monitor), and a Tiny Tapeout submission package that drops into the canonical `ttsky-verilog-template` layout (so the same graph can become a real ASIC on the next Sky130 or GF180 shuttle). The 40 blocks include the seven CPU primitives Adder / Subtractor / Comparator / Mux / Register / RAM / ROM and the Reinterpret bridge, so a branchable accumulator machine wires up on the canvas and the running sum drives audio. There's an optional AI consultant (bring your own Anthropic API key) that can read the canvas and wire things up with a preview-confirm step.
