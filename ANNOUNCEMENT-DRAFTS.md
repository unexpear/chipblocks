...
36 blocks in this alpha (the audio set plus a 5-block visual set: VGA Timing, Color Bars, Pixel Range, Solid Color, VGA Output that drive a real monitor through the iCEBreaker's PMOD1B socket, the Bus Split / Bus Join pair for cross-width signal routing, and the four CPU primitives Adder / Register / RAM / ROM that let you wire the data-path of a tiny accumulator machine). There's also an AI consultant in the sidebar (bring your own Anthropic API key) that can read your canvas, suggest blocks, and wire things up for you with a preview-and-confirm step.
...
Repo + installer: https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.5
...
First alpha of ChipBlocks: a free open-source desktop app that puts a visual node-graph editor on top of the open iCE40 toolchain. You wire 36 blocks (oscillators in four shapes, wavetable, noise, constant, mixer, ADSR, gate, sample-and-hold, multiply, FM voice, low/high/band-pass filters, bitcrusher, delay, distortion, AND/OR/XOR/NOT/counter logic primitives, VGA Timing / Color Bars / Pixel Range / Solid Color / VGA Output for a real video path on the iCEBreaker, Bus Split / Bus Join for cross-width composition, Adder / Register / RAM / ROM CPU primitives — 8-bit data + 4-bit address — for tiny accumulator machines, output) on a canvas, the backend translates the graph to Verilog via Amaranth HDL, and runs it through Yosys → nextpnr-ice40 → icepack to produce a flashable `.bin`. There's also an audio simulation path that produces a WAV from the Amaranth design so you can hear it before flashing.
...
First alpha covers audio / synth / retro-game chips — 36 blocks across oscillators (square / triangle / sawtooth / sine / wavetable), modulation (gate, ADSR, sample-and-hold, multiply, FM voice), filtering (low/high/band-pass), effects (bitcrusher, delay, distortion), digital logic (AND, OR, XOR, NOT, counter), visual (VGA Timing, Color Bars, Pixel Range, Solid Color, VGA Output — drives a real monitor on the iCEBreaker FPGA's PMOD1B header), bus composition (Bus Split, Bus Join — fan an 8-bit bus out to 8 individual 1-bit signals or concat back), CPU primitives (Adder, Register, RAM, ROM — 8-bit data + 4-bit address; data-path of a tiny accumulator machine), and routing (mixer, output, noise, constant). MIT-licensed, ships only permissively-licensed code (MIT / Apache 2.0 / BSD / ISC). Pipeline is Amaranth HDL → Verilog → Yosys → nextpnr-ice40 → icepack for the FPGA paths; Tiny Tapeout's flow runs OpenLane on Sky130 or GF180.
...
https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.5
...
ChipBlocks is a free open-source desktop app that lets non-engineers design custom chips by dragging visual blocks. The first alpha covers audio / synth / retro-game chips — 36 blocks in a node-graph editor, simulated audio output, plus four real-silicon outputs: a flashable iCE40 bitstream for the $30 Lattice iCEstick, a bitstream for the TinyFPGA BX, a bitstream for the 1BitSquared iCEBreaker (~$70 with PMOD headers — and a working VGA path on PMOD1B: drop in a $8 VGA-PMOD attachment, build the bundled `color-bars.json` graph, see 8 SMPTE color bars on a monitor), and a Tiny Tapeout submission package that drops into the canonical `ttsky-verilog-template` layout (so the same graph can become a real ASIC on the next Sky130 or GF180 shuttle). The 36 blocks now include the four CPU primitives Adder / Register / RAM / ROM, so a tiny accumulator machine wires up on the canvas. There's an optional AI consultant (bring your own Anthropic API key) that can read the canvas and wire things up with a preview-confirm step.
