...
27 blocks in this alpha (the audio set plus a 3-block visual set: VGA Timing, Color Bars, VGA Output that drive a real monitor through the iCEBreaker's PMOD1B socket). There's also an AI consultant in the sidebar (bring your own Anthropic API key) that can read your canvas, suggest blocks, and wire things up for you with a preview-and-confirm step.
...
Repo + installer: https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.2
...
First alpha of ChipBlocks: a free open-source desktop app that puts a visual node-graph editor on top of the open iCE40 toolchain. You wire 27 blocks (oscillators in four shapes, wavetable, noise, constant, mixer, ADSR, gate, sample-and-hold, multiply, FM voice, low/high/band-pass filters, bitcrusher, delay, AND/OR/XOR/NOT/counter logic primitives, VGA Timing / Color Bars / VGA Output for a real video path on the iCEBreaker, output) on a canvas, the backend translates the graph to Verilog via Amaranth HDL, and runs it through Yosys → nextpnr-ice40 → icepack to produce a flashable `.bin`. There's also an audio simulation path that produces a WAV from the Amaranth design so you can hear it before flashing.
...
First alpha covers audio / synth / retro-game chips — 27 blocks across oscillators (square / triangle / sawtooth / sine / wavetable), modulation (gate, ADSR, sample-and-hold, multiply, FM voice), filtering (low/high/band-pass), effects (bitcrusher, delay), digital logic (AND, OR, XOR, NOT, counter), visual (VGA Timing, Color Bars, VGA Output — drives a real monitor on the iCEBreaker FPGA's PMOD1B header), and routing (mixer, output, noise, constant). MIT-licensed, ships only permissively-licensed code (MIT / Apache 2.0 / BSD / ISC). Pipeline is Amaranth HDL → Verilog → Yosys → nextpnr-ice40 → icepack for the FPGA paths; Tiny Tapeout's flow runs OpenLane on Sky130 or GF180.
...
https://github.com/unexpear/chipblocks/releases/tag/v0.1.0-alpha.2
