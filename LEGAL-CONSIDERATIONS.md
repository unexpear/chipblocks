...
- **Silkscreen outline coordinates** — a closer look (2026-07-05, prompted by the "don't want sued" review) upgrades this from "thin expression" to **merger-doctrine safe**. Silk must mark the body while clearing every pad by the standard clearance, so its position is *dictated by the pad geometry*, not chosen. Worked example: the 0603 silk is two short lines at y = ±0.5225 mm — the only band above/below the pads (pad edge at y = ±0.475 mm) — spanning only the x-gap between the pads. The number 0.237258 is a *computation* off the pad clearances, not a creative pick; any implementer honoring the same function + IPC-7351 clearances lands in the same place. When a functional requirement forces one expression, the **merger doctrine** makes that expression **uncopyrightable** — the same reason the pad dimensions are facts. So the silk is safe as-is (merger + attribution), not merely low-risk; regenerating it would only reproduce the same functionally-forced coordinates. Conclusion: **no change needed.**
...
Strong. The project's discipline of citing values per-fixture with sources, the explicit rule about not copying graphics from copyrighted symbol libraries, and the facts-based reproduction of standard-derived footprint dimensions address the main copyright risks. On the closer 2026-07-05 look, even the silkscreen coordinates are covered by the merger doctrine (their position is forced by the pad geometry), so there is no remaining non-fact reproduction — the footprint data is factual/functional throughout, and KiCad is attributed as the reproducible reference + ground-truth.
...
| KiCad-modeled silk OUTLINES | Merger doctrine (silk position forced by pad geometry) + attributed | Low → very low (functionally forced = uncopyrightable) | No change needed |
