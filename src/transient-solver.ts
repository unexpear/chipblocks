...
type InductionMotorDqElement = {
  ...
  iA: number | undefined
  iB: number | undefined
  hasAcDrive: boolean // an AC source shares the motor's circuit (gates the inrush wording)
  core: DqMotorCore
}

/**
 * An AC induction motor resolved for the time loop: the dq (stationary-frame) dynamic model —
 * the rotor genuinely SPINS UP from rest (see induction-motor-dq.ts for the physics). The two
 * unseen phases of the balanced machine are the port waveform delayed by thirds of the drive
 * period, so the model is honest at any drive frequency or waveform; at DC it degenerates to
 * exactly the stator R1 the DC solver stamps (the two engines can never disagree). Without a
 * rotor_inertia the rotor is HELD at the nameplate operating speed — the old quasi-static
 * behavior, kept so saved circuits keep solving — and the caller is told in a warning.
 */
function resolveInductionMotorDq(
  inst: Instance,
  nodeIndex: Map<string, number>,
  world: World,
  dt: number,
): {
  element?: InductionMotorDqElement
  stalled: boolean
  notes: string[]
  ...
  const c2 = inst.connects[1]
  if (c1 === undefined || c2 === undefined) return null
  // A conducting stator and a real magnetizing branch; leakages small but never negative. (The
  // params reader already guarantees f > 0, poles > 0, R2 > 0.)
  ...
  let driveHz = p.frequency
  if (acFrequencies.size === 1) {
    driveHz = [...acFrequencies][0] ?? p.frequency
  } else if (acFrequencies.size > 1) {
    notes.push(
      `Induction motor '${inst.id}': multiple AC source frequencies share its circuit — its ` +
        `unseen phases are synthesized at the nameplate period (${p.frequency} Hz)`,
    )
  }
  const period = 1 / driveHz

  // With a step at or past a third of the period the delayed-phase lookup reads stale samples
  // and the synthesized phases silently stop being a three-phase set — refuse, honestly. (A
  // pure-DC drive has no waveform to mis-sample, so it needs no refusal.)
  if (acDrives.length > 0 && dt >= period / 3) {
    notes.push(
      `Skipped induction motor '${inst.id}': the time step (${dt} s) is not below a third of ` +
        `the drive period (${period.toPrecision(3)} s) — the unseen-phase synthesis needs finer ` +
        'time resolution; use a smaller time step',
    )
...
/** A shaded-pole induction motor element — the shorted-shading-ring machine (main + ring + rotor,
 *  a 4-state coupled model), a one-port Norton companion (induction-motor-shaded-pole.ts). */
type InductionMotorShadedPoleElement = {
  id: string
  termA: string
  termB: string
  netA: string
...
function resolveInductionMotorShadedPole(
  inst: Instance,
  nodeIndex: Map<string, number>,
  world: World,
): {
  element?: InductionMotorShadedPoleElement
  stalled: boolean
  notes: string[]
  acDrives: { hz: number; rms: number }[]
} | null {
  const p = shadedPoleMotorParamsFromInstance(inst)
  if (p === undefined) return null
  if (inst.connects?.length !== 2) return null
  const c1 = inst.connects[0]
  const c2 = inst.connects[1]
  if (c1 === undefined || c2 === undefined) return null
  const inductances = shadedPoleInductancesFromParams(p)
  if (inductances === null) return null // a singular / non-physical inductance matrix
  const notes: string[] = []
  const op = shadedPoleOperatingPoint(p)
  const acDrives = connectedAcDrives(world, [c1.net, c2.net])
  const acFrequencies = new Set(acDrives.map((d) => d.hz))
  const driveHz = acFrequencies.size === 1 ? ([...acFrequencies][0] ?? p.frequency) : p.frequency
  const rotorInertia = readScalarParam(inst, 'rotor_inertia')
  ...
  const wSync = synchronousSpeedRadPerSec(p.frequency, p.poles)
  // Quasi-static fallback: hold at the nameplate running speed only when an AC drive shares the
  // circuit; a pure DC drive cannot run it — hold at rest so the DC port is exactly R1, matching
  // the DC solver's stamp (the engines must never disagree on DC).
  const lockedOmega = acDrives.length > 0 ? (1 - op.slip) * wSync : 0
  return {
    stalled: op.stalled,
    notes,
    ...
    element: {
      ...
      iB: nodeIndex.get(c2.net),
      hasAcDrive: acDrives.length > 0,
      core: createShadedPoleMotor(inductances, mechanics, lockedOmega, 1 / driveHz),
    },
  }
}
...
/**
 * Stamp an induction motor's dq companion for the step ending at time t: the exact Norton
 * i_a = G·v_a + I_hist of the trapezoidal flux update at frozen rotor speed (a linear,
 * provably passive one-port — see induction-motor-dq.ts).
 */
function stampDqMotorCompanion(
  m: InductionMotorDqElement,
  t: number,
  dt: number,
  // biome-ignore lint/suspicious/noExplicitAny: mathjs Matrix is polymorphic
  M: any,
  ...
  const { conductance, historyCurrent } = dqStampMotor(m.core, t, dt)
  const { iA, iB } = m
  if (iA !== undefined) {
...
/** Stamp the shaded-pole motor's one-port Norton — same one-port shape as the sp sibling. */
function stampShadedPoleMotorCompanion(
  m: InductionMotorShadedPoleElement,
  dt: number,
  // biome-ignore lint/suspicious/noExplicitAny: mathjs Matrix is polymorphic
  M: any,
  // biome-ignore lint/suspicious/noExplicitAny: mathjs Matrix is polymorphic
  ...
  const { conductance, historyCurrent } = shadedPoleStampMotor(m.core, dt)
  const { iA, iB } = m
  if (iA !== undefined) {
    M.set([iA, iA], (M.get([iA, iA]) ?? 0) + conductance)
    b.set([iA, 0], (b.get([iA, 0]) ?? 0) - historyCurrent)
...
export function solveTransient(inputWorld: World, options: TransientOptions): TransientResult {
  ...
  const shadedPoleMotors: InductionMotorShadedPoleElement[] = []
  ...
  for (const inst of world.instances.values()) {
    ...
    if (inst.definition === 'power_source' && inst.connects?.length === 2) {
      ...
      // The dq DYNAMIC model — flux + rotor-speed states behind a Norton companion at the port
      // (induction-motor-dq.ts). The rotor genuinely spins up when a rotor_inertia is given.
      const im = resolveInductionMotorDq(inst, nodeIndex, world, dt)
      if (im === null) {
        warnings.push(
          `Skipped induction motor '${inst.id}' (needs both terminals wired and physical equivalent-circuit values: R1 > 0, Xm > 0, X1/X2 >= 0 with X1 + X2 > 0)`,
        )
      } else {
        warnings.push(...im.notes)
        if (im.element !== undefined) {
          dqMotors.push(im.element)
          if (im.stalled) {
            warnings.push(
      ...
      // The shaded-pole machine: a shorted shading ring (no cap, no switch), the direct
      // main↔shading coupling giving a weak self-start (induction-motor-shaded-pole.ts).
      const im = resolveInductionMotorShadedPole(inst, nodeIndex, world)
      if (im === null) {
        warnings.push(
          `Skipped shaded-pole induction motor '${inst.id}' (needs both terminals wired and physical winding values: R1/R2/R3 > 0, Xm > 0, 0 < X13 < Xm, leakage reactances >= 0)`,
        )
      } else {
        warnings.push(...im.notes)
        if (im.element !== undefined) {
          shadedPoleMotors.push(im.element)
          if (im.stalled) {
            warnings.push(
              `Induction motor '${inst.id}' is STALLED — the load exceeds what the running machine can carry`,
            )
  ...
  const solveInstant = (
    ...
    if (mode === 'initial') {
      ...
      for (const m of shadedPoleMotors) stampFixedCurrent(m.iA, m.iB, 0, b)
      ...
      for (const m of shadedPoleMotors) stampShadedPoleMotorCompanion(m, dt, M, b)
  ...
  const solveConverged = (
    ...
    for (let iter = 1; iter <= iterCap; iter++) {
      ...
      for (const d of diodes) acc.add(updateJunctionGuess(d, volts))
      ...
      for (const sh of shockleyDiodes)
        if (sh.state === 'conducting') acc.add(updateJunctionGuess(sh.diode, volts))
      ...
      // A varactor integrates its charge Q(V) implicitly, so its Newton guess is just this instant's
      // raw junction voltage (no limiting) — a transient-only device, not part of the shared DC loop.
      for (const cap of caps) {
        if (cap.varactor === undefined) continue
        const next = volts(cap.netA) - volts(cap.netB)
  ...
  const recordCurrents = (
    ...
    for (const m of shadedPoleMotors) {
      const stamp = m.core.stamp
      const amps =
        mode === 'initial' || stamp === null
          ? 0
  ...
  for (const m of shadedPoleMotors) {
    shadedPoleInitMotor(m.core, (initial.nodes.get(m.netA) ?? 0) - (initial.nodes.get(m.netB) ?? 0))
  }
  ...
  for (let k = 1; k <= steps; k++) {
    ...
    for (const m of dqMotors) {
      // Commit the converged step: fluxes + i₀ from the solved port voltage, the developed
      // torque, the rotor speed step, and the port sample onto the phase delay line. The rising
    ...
    for (const m of shadedPoleMotors) {
      shadedPoleCommitMotor(m.core, (nodes.get(m.netA) ?? 0) - (nodes.get(m.netB) ?? 0), dt)
    }
  ...
  type StuckCore = DqMotorCore | Dq3MotorCore | SpMotorCore | ShadedPoleMotorCore
  const stuckMotors: { id: string; hasAcDrive: boolean; core: StuckCore }[] = [
    ...
    ...shadedPoleMotors.map((m) => ({
      id: m.id,
      hasAcDrive: m.hasAcDrive,
      core: m.core as StuckCore,
    })),
  ]
  for (const m of stuckMotors) {
    ...
    if (m.core.omega !== 0) continue
    warnings.push(
      m.hasAcDrive
        ? `Induction motor '${m.id}' did NOT start — its developed torque never exceeded the ` +
            'shaft load; the rotor sits at standstill drawing the locked-rotor current'
        : `Induction motor '${m.id}' did NOT start — a DC drive makes no rotating field and no ` +
            'torque; the winding just draws its stator-resistance current',
    )
  }
