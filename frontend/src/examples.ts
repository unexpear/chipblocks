...
export const EXAMPLES: ExampleGraph[] = [
  ...
  {
    id: 'cpu-accumulator',
    label: 'CPU accumulator (4 new primitives)',
    description: 'A tiny accumulator using the Sprint 17 CPU primitives: a Counter walks a 16-entry ROM, the ROM byte feeds Adder.in-a, a Register holds the running sum and feeds it back via Adder.in-b, and a parallel RAM logs each new sum to the same address. CPU-domain only — the silent audio Output is just so the synth pipeline has an Output sink. The ROM holds the first 8 Fibonacci numbers padded with zeros.',
    nodes: [
      ...
      { id: 'k_audio', type: 'constant', position: { x: 40,  y: 320 }, data: { value: 0 } },
      { id: 'out',     type: 'output',   position: { x: 240, y: 320 }, data: {} },
    ],
    edges: [
      ...
      { id: 'e10', source: 'k_audio', target: 'out', sourceHandle: 'audio-out', targetHandle: 'audio-in'     },
    ],
  },
]
