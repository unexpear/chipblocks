...
export type BitcrusherBlock = Node<{ bits: number }, 'bitcrusher'>

export function BitcrusherNode({ id, data }: NodeProps<BitcrusherBlock>) {
  const { updateNodeData } = useReactFlow()

  const commit = useCallback(
    (v: number) => updateNodeData(id, { bits: v }),
    [id, updateNodeData],
  )
    ...
    <div className="block block-bitcrusher" role="group" aria-labelledby={titleId}>
      ...
      <div className="block-body">
        <input
          ...
          max={8}
          step={1}
          aria-label="Effective bit depth (1 to 8)"
