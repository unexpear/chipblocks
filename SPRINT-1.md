...
| **P0** | **3. Electron + React + TypeScript scaffold** — working desktop app that opens a window saying "ChipBlocks" with a basic React UI; `npm run dev` works | 2–3 hrs | Claude Code | Item 1 |
...
**✓ Done — 2026-05-07.** Scaffolded from `electron-vite-react` boilerplate (MIT, cloned via `degit electron-vite/electron-vite-react`) into `frontend/`. Stack: Electron 33, React 18, TypeScript 5.4, Vite 5.4, Tailwind 3.4. Customized: window title and HTML `<title>` → "ChipBlocks"; `App.tsx` replaced with minimal landing + counter (HMR proof); removed auto-updater import and call from main process; removed boilerplate `.github/` CI templates, Chinese readme, sample `.txt` configs, and ~14 MB of marketing GIFs. `npm install` completed cleanly, `npx tsc --noEmit` passes. **To run**: `cd frontend && npm run dev` — opens an Electron window titled "ChipBlocks". Auto-updater UI files (`src/components/update/`, `electron/main/update.ts`) left in place but unused; can clean up later if desired.
