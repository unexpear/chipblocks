...
- **42 blocks**: oscillator (square), triangle, sawtooth, sine, noise, constant, mixer, output, ADSR envelope, gate, low-pass filter, high-pass filter, band-pass filter, sample-and-hold, FM voice, multiply (ring modulator), wavetable, bitcrusher, delay, distortion (hard-clipping waveshaper), the digital-logic primitives AND / OR / XOR / NOT / counter, the visual blocks VGA Timing / Color Bars / Pixel Range / Solid Color / VGA Output that drive a monitor through an iCEBreaker FPGA + VGA-PMOD attachment, bus-composition blocks Bus Split / Bus Join (8-bit ↔ 8 × 1-bit fan-out / concat) for cross-width signal routing, the CPU primitives Adder / Subtractor / Comparator / Mux / Register / RAM / Register File / ROM (8-bit data + 4-bit address; Register File has independent read and write address ports for CPU-style register addressing) for building tiny accumulator machines and branchable programs, a Byte Constant block for CPU-domain literals, and a Reinterpret bridge (data-u8 → audio-s8) so the CPU domain can drive audio. Drag from the side palette onto the canvas.
