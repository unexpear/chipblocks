...
Read [PRD.md](PRD.md) for the full vision. Read [ROADMAP.md](ROADMAP.md) for what's next. Per-sprint plans + retros live in [SPRINT-1.md](SPRINT-1.md) through [SPRINT-20.md](SPRINT-20.md). Status as of 2026-05-10: 19 sprints closed plus Sprint 20 in flight. v0.1.0-alpha capability-complete (42 blocks — the 32 from the alpha plus the Sprint 17 CPU primitives Adder / Register / RAM / ROM, the Sprint 18 conditional-control trio Subtractor / Comparator / Mux + the Reinterpret bridge that closes the data-u8 ↔ audio-s8 sign-class barrier, and the Sprint 20 Register File with independent read/write addresses). Pending user-action launch gates (tag push + announcements + Discussions enable).
...
### Testing
- **Backend**: pytest under `backend/tests/` — 63 tests + 2 skipped: 47 property-based block tests covering all 42 blocks (including the 5 visual blocks, the 2 bus-composition blocks, the 7 CPU primitives Adder / Subtractor / Comparator / Mux / Register / RAM / ROM, the Sprint 20 Register File with its independent-port round-trip + multi-port pipeline smoke, the Reinterpret bridge, the Counter.addr-out extension, and three pipeline smoke tests total), 9 pipeline tests against the example graphs (3 of which exercise the visual path), 8 Tiny Tapeout submission-package tests. Run via `python3 -m pytest backend/tests/ -v` from WSL2 (~110 s).
- **Frontend**: vitest under `frontend/test/` — 161 tests covering IPC contracts (synth/build/AI), block-component rendering + parameter editing (now including all 42 blocks), bus-type compatibility, error classification, save/load roundtrip, examples-consistency (14 bundled graphs), cross-registry alignment (PALETTE / BLOCK_PORT_TYPES / nodeTypes / AI prompt all match). Run via `cd frontend && npm test` (~9 s).
...
- [ADR-003-block-manifest.md](ADR-003-block-manifest.md) — block-manifest refactor for Sprint 21. **Status: Proposed.** Adopt `blocks.yaml` as single source of truth + 2 codegen scripts (Node + Python) writing 7 generated sections delimited by `@begin codegen` / `@end codegen` markers. Cuts per-block hand-edited surface from 9 files to 3. ~5 h estimate.
