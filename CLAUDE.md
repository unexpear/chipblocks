...
Read [PRD.md](PRD.md) for the full vision. Read [ROADMAP.md](ROADMAP.md) for what's next. Per-sprint plans + retros live in [SPRINT-1.md](SPRINT-1.md) through [SPRINT-13.md](SPRINT-13.md). Status as of 2026-05-09: 13 sprints closed; v0.1.0-alpha capability-complete (27 blocks including the VGA Timing / Color Bars / VGA Output visual trio that drives a monitor through the iCEBreaker FPGA's PMOD1B header); pending user-action launch gates (tag push + announcements + Discussions enable).
...
### Testing
- **Backend**: pytest under `backend/tests/` — 45 tests total: 28 property-based block tests covering all 27 blocks (including the 3 visual blocks and one mixed-logic pipeline smoke test), 9 pipeline tests against the example graphs (3 of which exercise the visual path: a friendly-error rejection on ▶ Play, the iCEBreaker .pcf carrying VGA pin assignments, and an end-to-end build of the color-bars graph to a real iCEBreaker bitstream), 8 Tiny Tapeout submission-package tests. Run via `python3 -m pytest backend/tests/ -v` from WSL2 (~60 s).
- **Frontend**: vitest under `frontend/test/` — 98 tests covering IPC contracts (synth/build/AI), block-component rendering + parameter editing (now including the 3 visual blocks), error classification, save/load roundtrip. Run via `cd frontend && npm test` (~8 s).
...
- [BLOCKS.md](BLOCKS.md) — block library reference: per-block ports, parameters, behavior, common-usage notes for all 27 blocks (including the new "Visual" section covering VGA Timing / Color Bars / VGA Output)
