...
Read [PRD.md](PRD.md) for the full vision. Read [ROADMAP.md](ROADMAP.md) for what's next. Per-sprint plans + retros live in [SPRINT-1.md](SPRINT-1.md) through [SPRINT-13.md](SPRINT-13.md). Status as of 2026-05-09: 13 sprints closed; v0.1.0-alpha capability-complete (30 blocks including the VGA Timing / Color Bars / Pixel Range / Solid Color / VGA Output visual quintet that drives a monitor through the iCEBreaker FPGA's PMOD1B header, and a Distortion waveshaper for overdriven synth tones); pending user-action launch gates (tag push + announcements + Discussions enable).
...
### Testing
- **Backend**: pytest under `backend/tests/` — 48 tests total: 31 property-based block tests covering all 30 blocks (including the 5 visual blocks and one mixed-logic pipeline smoke test), 9 pipeline tests against the example graphs (3 of which exercise the visual path: a friendly-error rejection on ▶ Play, the iCEBreaker .pcf carrying VGA pin assignments, and an end-to-end build of the color-bars graph to a real iCEBreaker bitstream), 8 Tiny Tapeout submission-package tests. Run via `python3 -m pytest backend/tests/ -v` from WSL2 (~60 s).
- **Frontend**: vitest under `frontend/test/` — 103 tests covering IPC contracts (synth/build/AI), block-component rendering + parameter editing (now including the 5 visual blocks), error classification, save/load roundtrip. Run via `cd frontend && npm test` (~8 s).
...
- [BLOCKS.md](BLOCKS.md) — block library reference: per-block ports, parameters, behavior, common-usage notes for all 30 blocks (including the "Visual" section covering VGA Timing / Color Bars / Pixel Range / Solid Color / VGA Output)
