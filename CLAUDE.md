...
- **Adding a block**: edit `blocks.yaml` and the two `.tsx` + `.py` files; run `npm run codegen` from `frontend/`. CI catches drift. See [ADR-003](ADR-003-block-manifest.md) for the manifest design.
...
### Testing
- **Backend**: pytest under `backend/tests/` — 189 tests + 2 skipped: 47 property-based block tests covering all 42 blocks (including the Sprint 20 Register File with its independent-port round-trip + multi-port pipeline smoke; the 7 CPU primitives Adder / Subtractor / Comparator / Mux / Register / RAM / ROM; the Reinterpret bridge; the Counter.addr-out extension; and three pipeline smoke tests total), 9 pipeline tests against the example graphs (3 of which exercise the visual path), 8 Tiny Tapeout submission-package tests, plus 126 dynamic manifest-integrity assertions from Sprint 21 (42 blocks × 3 invariants — `backendPath` file exists, `backendClass` importable, registered in `BLOCK_REGISTRY` with matching `__name__`). Run via `python3 -m pytest backend/tests/ -v` from WSL2 (~85 s).
- **Frontend**: vitest under `frontend/test/` — 287 tests covering IPC contracts (synth/build/AI), block-component rendering + parameter editing (now including all 42 blocks), bus-type compatibility, error classification, save/load roundtrip, examples-consistency (14 bundled graphs), cross-registry alignment (PALETTE / BLOCK_PORT_TYPES / nodeTypes / AI prompt all match), plus 126 dynamic manifest-integrity assertions from Sprint 21 (same 42 × 3 shape — `componentPath` file exists, exports `${PascalCase}Node`, registered in `nodeTypes`). Run via `cd frontend && npm test` (~9 s).
...
- [BLOCKS.md](BLOCKS.md) — block library reference: per-block ports, parameters, behavior, common-usage notes for all 42 blocks (including the "Visual" section covering VGA Timing / Color Bars / Pixel Range / Solid Color / VGA Output, the "Bus" section covering Bus Split / Bus Join / Reinterpret, and the "Computation" section covering Adder / Subtractor / Comparator / Mux / Register / RAM / Register File / ROM)
