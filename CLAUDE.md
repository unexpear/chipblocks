...
- **ADR-005 — Modular fab platform** (draft pending; not yet written). Apply the ADR-003 manifest pattern to the fab target itself. Eight extension points: `shuttles.yaml` / `pdks.yaml` / `cpu-cores.yaml` / `radios.yaml` / `buses.yaml` / `memories.yaml` / `packages.yaml` / `flows.yaml`. Each addable as 1 row + 1 adapter. First implementation step: materialise `shuttles.yaml` with the existing Tiny Tapeout slot as row 1 (Sprint 25), proving the pattern end-to-end before any new shuttle tier ships. **Status: planned for Sprint 25 draft.**
