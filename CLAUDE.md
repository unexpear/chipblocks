...
**Current status (2026-06-11, Sprint 19 closed):** a working Electron + React + TypeScript app lives on master. Shipped so far: JSON-Schema-validated catalog (materials, behaviors, interfaces, primitive devices through diodes, BJTs, MOSFETs, and transformers, all cited per the anti-placeholder rules) with a cross-FK validator; **two solvers** — DC (Modified Nodal Analysis + Newton-Raphson + pnjlim, with lumped thermal + electro-thermal feedback) and transient/time-domain (backward-Euler + per-step Newton-Raphson: R/C/L, AC sources, diodes/LEDs, NPN+PNP BJTs, NMOS/PMOS, transformers incl. center-tapped with core loss + saturation detection); a **full interactive circuit editor** (palette through MOSFETs, properties panel with cited defaults + live readings, CAD-style wire tool with curves and junctions, box/lasso selection with wires-by-touch, clipboard with 15-copy history, undo/redo, multi-lead sources that expand into real tapped stacks, **circuit blocks** — a drawn circuit grouped into one reusable part that always flattens to its real parts for the solver, rebindable shortcuts, themed scrollbars, unbounded zoom); a **multimeter tool** (probe terminals like real meter leads — DC volts, true-RMS AC volts + counted frequency, powered-off Ω + continuity, diode test, capacitance, clamp-style amps on wires; numbers verified against Fluke 117 documentation); a **complete oscilloscope** (pick-your-probes by clicking terminals, current clamps by clicking wires, edge trigger with Auto/Normal/Single + pretrigger, timebase + per-channel volts/div on a 10×8 graticule with honest-sampling refusals, draggable cursors, live auto-measurements sharing the meter's verified math, math channels with unit algebra — volts × clamp-amps reads REAL WATTS, a from-scratch FFT, persistence, XY mode, CSV export); a **Math panel** (every equation with the real numbers in it; KCL re-summed at every net — the ✓ computed, not assumed); five lenses (voltage/power/temp/flow/magnetic-field); failure-mode checks (LED, resistor, capacitor polarity/overvoltage, over-temperature, MOSFET gate overvoltage, diode reverse breakdown); circuit **Save/Load** (`.chipblocks` files); a native menu (File/Edit/View/Settings). 633 Vitest tests + tsc + Biome + build gate every commit. Sprint history lives in `sprints/` (Sprints 1–19 closed; 19 was the 84-increment interactive-canvas + simulation mega-sprint, increments `S19-v3-NN`). Older history: v1 on `legacy/audio-synth-direction`; v2 on `archive/foundation-pre-second-reset`.
