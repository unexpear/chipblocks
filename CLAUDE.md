...
- [MASK-AND-LAYOUT-EDA-ARC.md](MASK-AND-LAYOUT-EDA-ARC.md) — the chip-side "manufacturing data" arc: photolithography/mask fundamentals (Rayleigh CD = k₁·λ/NA), GDSII/OASIS layout formats + the real per-layer `name→{gds_layer,datatype}` map (with SKY130's actual numbers), the layout→mask flow + DRC/LVS tapeout sign-off (masks/OPC/fracturing = foundry-side; DRC/LVS-clean GDS = our deliverable, the Gerber analog), the open-source EDA + open-PDK landscape with **primary-source-verified licenses** (KEY: Magic permissive/bundleable but **Netgen is GPL/external-only** — not a pair; open PDKs SKY130/GF180/IHP all Apache-2.0), the current fab-shuttle landscape (Efabless closed 2025 → ChipFoundry/Tiny Tapeout/wafer.space), and the from-scratch buildable pieces + algorithms (Euler-path cell layout, GDSII writer, scanline DRC, Gemini-LVS) with a recommended build order. Verified 2026-07-17 via two deep-research passes (110-agent fundamentals + 5-searcher gap-fill, primary repo LICENSE files).
