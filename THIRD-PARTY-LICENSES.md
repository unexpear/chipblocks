...
#### Project IceStorm — iCE40 chipdb fragment
- **What:** `fixtures/icebox-ice40-384-fragment.chipdb` — a small verbatim fragment (a `.device` line plus a few `.net` / `.buffer` / `.routing` blocks) of a real Lattice iCE40 device ("384") chip database, generated by `icebox_chipdb.py -3`. Used as a test fixture proving the Stage-2 chipdb parser (`src/renderer/fpga-icebox.ts`) ingests genuine iCE40 data, not an invented format.
...
- **License text:** the ISC permission notice is preserved verbatim in the fixture file's header comment (as ISC requires — "the above copyright notice and this permission notice appear in all copies"). No separate LICENSE file is vendored; the full device chipdb (~2 MB) is **not** vendored — only the cited fragment.
- **Usage tier:** dev-time/test only (a test fixture; not shipped in the product artifact).
