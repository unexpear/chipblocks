...
- **Vendored data files:** `fixtures/trellis-ecp5-LFE5U-25F-tilegrid.json` (2.8 MB — the LFE5U-25F tile grid) and `fixtures/trellis-ecp5-PLC2-bits.db` (58 kB — the PLC2 logic-tile bit database), both copied unmodified from prjtrellis-db so `src/renderer/fpga-trellis-tiles.ts` can map frames to tiles and decode SLICE LUT4s. **CC0-1.0** places these in the public domain and imposes no notice requirement; attribution is recorded here anyway.
