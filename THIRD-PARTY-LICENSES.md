...
- **Note:** these are interoperability FACTS (opcode values, a checksum polynomial, per-part frame counts), transcribed into original TypeScript rather than copied as code or data files, so no notice file is redistributed. Attribution is recorded here and in the module's header comment.
