...
- **IEEE** — 315 (US schematic symbols), 60028 (cross-reference to IEC), **1364 (Verilog HDL)** — the language the HDL bridge implements a synthesizable structural subset of (`src/renderer/verilog.ts`); an open IEEE standard, and syntax/facts are not copyrightable
...
- **Icarus Verilog** (GPLv2), **Verilator** (LGPL-3.0 / Artistic-2.0 dual), **Yosys** (ISC — permissive) — the reference Verilog implementations the HDL bridge's emitter (and, later, parser + synthesizer) were cross-checked against for IEEE 1364-2005 structural conformance and portability. **No code from any is used or bundled** — ChipBlocks' Verilog reader/writer/synthesizer is written from scratch (a choice of the project's own-engine identity, not licensing; the permissive Yosys would be bundle-legal). Learning conventions/syntax from them is unrestricted (facts/ideas not copyrightable). Sources: [github.com/steveicarus/iverilog](https://github.com/steveicarus/iverilog), [github.com/verilator/verilator](https://github.com/verilator/verilator), [github.com/YosysHQ/yosys](https://github.com/YosysHQ/yosys). Verified 2026-07-13.
