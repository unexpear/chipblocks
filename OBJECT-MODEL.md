...
| **Multi-color LED catalog expansion.** Sprint 7 landed red AlGaInP only. The same material+behavior+device pattern applies to: blue InGaN (~450 nm, GaN buffer + InGaN active layer + GaN), green InGaN (~525 nm, same chemistry tuned), IR GaAs (~880-940 nm), UV AlGaN (~365 nm), white via blue InGaN + YAG:Ce phosphor. Each color is a small catalog addition (2 doped materials + 1 device definition + instance). Lands as a dedicated catalog sprint or rolls into Sprint 8+ semiconductor expansion. | Sprint 8+ catalog work |
