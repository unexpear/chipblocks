...
| ~~**Role-satisfaction validation.** When an instance fills a `composition.requires` role through a parameter (via `satisfies_role`), the validator must prove the selected object satisfies the role constraints — e.g., `copper` actually enables `electrical_conduction`. This is stricter than simple foreign-key existence.~~ ✅ **CLOSED in Sprint 3** (S3-v3-4). The cross-FK validator emits `role-unsatisfied` when the object chosen for a role doesn't enable every capability in the role's `must_enable`. Covered by tests (e.g., "chosen material does not enable the required capability"; the Sprint 9 "LED picks copper for n_side" case). | ✅ Closed Sprint 3 |
...
| ~~**PN junction as a separate interface kind.** Sprint 7's LED uses implicit PN junction via `composition.uses` listing both doped semiconductor materials directly.~~ ✅ **CLOSED in Sprint 9** (S9-v3-2). `pn_junction` is a first-class interface kind (`interface-pn-junction.yaml`): `composition.requires` n_side + p_side material roles with `must_enable [n_type_semiconductor]` / `[p_type_semiconductor]`. The generic `led` device composes it via `composition.uses: [pn_junction]`. Schottky junction (different physics) remains its own deferred row below. | ✅ Closed Sprint 9 |
...
| **Failure severity classification (warning / error / info).** Sprint 15's failure detector (§19) reports all violations as `severity: 'error'`. A richer model would distinguish soft thresholds — "approaching 80% of rating" = warning, "over rating" = error, "informational note" = info. Needs a design pass on the threshold model + how the UI surfaces each tier (color-coding on the canvas, ordering in a report). Lands when the canvas needs to visually distinguish failure severities. | When the canvas surfaces failures by severity |
| **Automatic fix suggestion.** Sprint 15's detector reports facts ("70 mA exceeds 20 mA"); it doesn't suggest "use a 470 Ω resistor instead." That's an inverse problem — solve for the component value that brings the circuit within ratings, given a goal (e.g., target 15 mA through the LED). Separate sprint; pairs naturally with an interactive canvas where the user can accept a suggested fix. | When interactive design assistance lands |
| **Additional electrical failure modes (wire ampacity, battery overcurrent, switch contact ratings).** Sprint 15 ships LED forward-overload, LED reverse-breakdown, and resistor overpower. The same pattern (read computed value from the Solution, compare to a rating parameter, fire a structured Failure) extends to wire current-carrying limits (AWG-dependent ampacity), battery overcurrent / internal-resistance sag, and switch contact-current ratings. Each lands when fixtures move into the relevant regime. | When fixtures exercise these regimes |
| **Transient (peak) ratings.** Some components have instantaneous peak ratings higher than their continuous ratings (e.g., a 100 ms surge current, an inrush spec). A DC solver computes only the operating point, so Sprint 15 can only check continuous (DC) ratings. Transient peak ratings need transient simulation (Stage 5 of the sim arc) and land with it. | Stage 5 — transient simulation |
| **Transistor nonlinear models (BJT Ebers-Moll, MOSFET).** Transistors are the next nonlinear devices after diodes. The companion-model + Newton-Raphson + pnjlim machinery built in §20 is the foundation they reuse — a BJT is roughly two coupled diode junctions; a MOSFET has its own I-V law (square-law or BSIM). Each needs its model equations + companion linearization + the device fixtures. Major content + solver effort; its own multi-sprint arc. | Future nonlinear-device arc |
| **Temperature-dependent thermal voltage V_T.** §20 fixes V_T at 300 K (25.852 mV). Real circuits run hot; V_T = kT/q rises with temperature, shifting diode curves. `thermalVoltage(T)` already takes T as a parameter — the hook exists. Lands with self-heating, which pairs with the thermal solver (Stage 7). The §16 `input_variable` mechanism is the natural way to thread temperature into equation-valued properties + the solver. | With the thermal solver (Stage 7) |
| **GMIN / source stepping (hard-convergence aids beyond pnjlim).** pnjlim (§20.5) handles the diode steep-region overflow, covering the anchor circuit and typical hobby circuits. Stiffer circuits (many diodes, tight feedback) can still fail to converge; SPICE's standard escalation is GMIN stepping (add a tiny parallel conductance, ramp it down) and source stepping (ramp sources up from zero). Sprint 16 returns honest `did-not-converge` instead; these aids land when a real circuit needs them. | When a real circuit fails to converge under pnjlim alone |
| **Dedicated rated-current parameter for I_s calibration.** §20.3 derives the diode saturation current I_s assuming `forward_voltage` is specified at `max_forward_current`. That holds for led_001 but isn't universal — many datasheets give V_F at a typical current below max. A dedicated "rated current at which V_F is specified" parameter would decouple the two and improve I_s accuracy. | When datasheet-accurate I_s calibration matters |
| **Power-port family beyond ground (Vcc / Vdd / named rails).** The Sprint 16 ground port (§18.2) is the reference-marker pattern for the 0 V net. The same pattern extends to supply rails: a Vcc / Vdd / +3V3 port that designates a net as a named supply, carrying the standard power-port symbol. The solver would seed those nets with their rail voltage. Lands when multi-rail circuits appear. | When multi-rail circuits appear |
| **Solver reads declared terminals + polarity hint.** Sprint 17 (§21) declares + validates terminals, but the DC solver still hardcodes terminal-name conventions in its stamp functions (`stampResistor` → `terminal_a`/`terminal_b`, `stampVoltageSource` → `terminal_positive`/`terminal_negative`, `stampLED` → `anode`/`cathode`, etc.). The follow-on: add an optional polarity / semantic-role hint to each terminal (`role: anode`, `polarity: positive`) and have the solver read the declared terminals + roles instead of hardcoding them — removing the duplication between the catalog's terminal names and the solver's strings, and making a new device's terminals a pure-data change. Lands when the duplication becomes a maintenance cost (a connectable device with non-standard terminal names forces it). | When the hardcoded-terminal duplication becomes a cost |
| **Electron packaging (electron-builder).** Sprint 18's canvas runs via `npm run dev` (electron-vite). Packaging into a distributable installer (electron-builder, MIT) — with Electron's bundled `LICENSES.chromium.html` (Chromium BSD + Node MIT) traveling per Electron's redistribution terms — is a later sprint. Pairs eventually with the two-deliverables / manufacturing-ZIP export. | A later canvas/release sprint |
| **Canvas interactivity.** Sprint 18 is a static render. Drag-to-move, place-component, draw-a-wire, right-click parameter override — the next big canvas effort, pairing with the auto-created-interface UX + right-click-override rows above. Needs canvas-only state persistence (`canvas/layout.yaml`) so user-moved positions + lock flags + color hints survive without triggering physics revalidation. | The interactivity canvas sprint |
| **Solver overlay on the canvas.** The high-value next canvas sprint: run `solveDC` (§18/§20) + `detectFailures` (§19) and paint the result onto the schematic — node voltages on nets, branch currents on wires, the overloaded LED highlighted red with its measured-vs-rated numbers. Everything it consumes is already built; it's a presentation layer over the solver + detector output. | The next canvas sprint |
| **Circuit-aware canvas layout.** Sprint 18 lays nodes out in a deterministic grid; edges cross. A readable schematic needs a real layout (auto-router, or saved manual positions via `canvas/layout.yaml`, or a topology-aware placement). Pairs with interactivity. | With canvas interactivity |
