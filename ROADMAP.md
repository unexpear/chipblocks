...
| **Mac + Linux installer build pipeline** (via GitHub Actions cross-platform CI, since the dev box is Windows-only) | 0.5–1 sprint | ~30% of likely audience | High | electron-builder is configured; CI is the missing piece. Unsigned is acceptable for alpha. |
...
| **IPC layer regression test** (carryforward; promote alongside CI work — same infrastructure) | 0.5 sprint | infrastructure | High | Was 6 sprints stale. The CI sprint is the natural home for it; running tests as part of the same pipeline. **This reverses the earlier "drop" call.** |
