...
| ~~**Tiny Tapeout submission package**~~ | — | — | — | **✓ Done in Sprint 10 + S10 fix.** `--target tt` produces a 14-file bundle in canonical `ttsky-verilog-template` layout (src/, test/, docs/, info.yaml, README.md, LICENSE, .gitignore, SUBMIT.md, plus a working cocotb testbench). Drop-in ready: unzip on the GitHub template, push, submit. Sample-rate divider baked into the TT wrapper so audio runs at 44.1 kHz on the cohort's 50 MHz clock. Active cohorts: **TTSKY26a closes 2026-05-11**, TTGF26a closes 2026-06-22. |
